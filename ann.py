...
def make_model(Nq, tgt_vocab, N=6, d_model=512, d_ff=2048, h=8, dropout=0.1):
...
def TrainModel(model, train_data_np, test_data_np, device, losses=[], smoothing=0.0, lr=0.001, batch_size=100, Nep=20):

    V = model.Na+3

    # Train Model
    losses0 = np.zeros((Nep),dtype=float)
    ...
    for epoch in range(Nep):
        ...
        scheduler.step()
        losses0[epoch] = epoch_loss

    train_data.to('cpu')
    test_data.to('cpu')
    return model, np.concatenate((np.array(losses), losses0))
