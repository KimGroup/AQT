...
def GetDMElement(st1, st2, ptab, Nq, povm):
    Na = povm.Na
    TinvM = povm.TinvM
    ...
    for n in range(Na**Nq):
        ...
        for nq in range(Nq):
            pa_elem[n] *= TinvM[o[nq], st1_vec[nq], st2_vec[nq]]
    return np.sum(pa_elem)


def GetDMSample(povm, samples, states=[], verbose=False):
    ...
    Ns = len(samples)
    pa = np.array([1./Ns for i in range(Ns)], dtype=float)


    for st1 in range(2**Nq):
        for st2 in range(st1, 2**Nq):
            st1_vec = int2basestr(st1, 2, l=Nq)
            st2_vec = int2basestr(st2, 2, l=Nq)

            pa_elem = np.array(pa, dtype=complex)
            for n in range(len(samples)):
                for nq in range(Nq):
                    pa_elem[n] *= TinvM[samples[n, nq], st1_vec[nq], st2_vec[nq]]
            dm[st1, st2] = np.sum(pa_elem)
            dm[st2, st1] = np.conj(dm[st1, st2])
